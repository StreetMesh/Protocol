export const metadata = {
  title: 'Introduction',
  description:
    'How the Web arrived at information, participation and decentralization — and what a spatial Web is for.',
}
...
By the mid-2000s the Web had a problem it had never had before: nobody knew what you were looking at it with. A page written for a 1024-pixel monitor arrived on a phone, a tablet, a television, and a laptop, and it could not ask which one it was.

The common answer at the time was to build a second website—the dreaded `m.` subdomain—and keep two copies of everything in step. In 2010, [Ethan Marcotte](https://alistapart.com/article/responsive-web-design/) proposed something better: one document that adapts. Fluid grids that measure in proportions instead of pixels, images that scale to their container, and [media queries](https://en.wikipedia.org/wiki/Media_queries) that let a stylesheet ask how much room it has been given and answer accordingly.

The idea that stuck was not the technique but the posture behind it. A responsive page does not decide what it looks like. It describes what it *is*—a heading, a list, a figure and its caption—and then negotiates the presentation with whatever is on the other end. The author gives up control over the exact result in exchange for the page working somewhere they never anticipated.

That trade is the one a spatial Web has to make again, and harder. A place will be entered from a phone held at arm's length, a laptop on a train, and a headset in someone's living room, by people standing up and sitting down and by machines that are not looking at it at all. Responsive design taught the Web to adapt to the size of a screen. What comes next has to adapt to the shape of a body, the reach of a hand, and the amount of attention someone can spare.
...
If a place is going to contain things, the things need a file format—and for most of the Web's history, 3D did not have one. It had dozens, each tied to the tool that wrote it, and moving a model between two programs meant losing its materials, its animation, or both.

[glTF](https://en.wikipedia.org/wiki/GlTF) is the format that settled it. Maintained by the [Khronos Group](https://www.khronos.org/gltf/), the same standards body behind OpenGL and WebGL, it is royalty-free, openly specified, and designed to be handed to a graphics card rather than translated first. Its authors call it "the JPEG of 3D," and the comparison is doing real work: JPEG did not win because it was the best way to store a picture, but because everything could open one.

A glTF file carries the whole object—geometry, materials, textures, skeleton, and animation—either as a folder of parts or as a single self-contained `.glb`. A browser can load one over plain HTTP, from any address, with no plugin and no runtime that belongs to a particular vendor.

That last part is why it matters here. A spatial Web where a chair, a sculpture, or an avatar can only exist inside the platform that made it is not a Web at all—it is a set of showrooms. Open standards for identity let a *person* move between places. Open standards for geometry let their *belongings* come with them.
